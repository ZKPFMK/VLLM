...
impl Shape {
    ...
    fn validate(self) {
        assert!(self.sequence_length > 0, "MLP expansion join sequence length must be nonzero");
        assert!(self.hidden_size > 0, "MLP expansion join hidden size must be nonzero");
        assert!(self.expansion_size > 0, "MLP expansion join width must be nonzero");
        assert!(self.num_tiles > 0, "MLP expansion join tile count must be nonzero");
        assert_eq!(
            self.expansion_size,
            3 * self.hidden_size,
...
fn parse_arguments() -> Arguments {
    ...
    let mut tile_dir = std::env::var_os("SP1_ZKGPT_MLP_EXPANSION_OUTPUT_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("sp1-zkgpt-mlp-expansion-output"));
    let mut output_dir = std::env::var_os("SP1_ZKGPT_MLP_EXPANSION_JOIN_DIR").map(PathBuf::from);
...
fn parse_tile_manifest(path: &Path) -> TileManifest {
    ...
    assert_eq!(required_field(&fields, "stage", path), "mlp_expansion_gelu_tile");
...
fn split_expansion_output(shape: Shape, combined: &[u16]) -> Vec<Vec<u16>> {
    assert_eq!(combined.len(), shape.sequence_length * shape.expansion_size);
    let tile_width = shape.tile_width();
    let mut tiles = vec![Vec::with_capacity(shape.sequence_length * tile_width); shape.num_tiles];
    for token_row in combined.chunks_exact(shape.expansion_size) {
...
fn concatenate_expansion_output(shape: Shape, tiles: &[JoinTileData]) -> Vec<u16> {
...
fn validate_join_data(shape: Shape, layer: usize, data: &JoinData) -> GroupCommitments {
    assert_eq!(data.tiles.len(), shape.num_tiles, "MLP expansion join requires every tile");
    ...
    for (expected_tile, tile) in data.tiles.iter().enumerate() {
        assert_eq!(tile.tile, expected_tile, "MLP expansion tiles must be ordered from zero");
        let output = host_commit_u16(DOMAIN_MLP_EXPANSION_TILE_OUTPUT, &tile.output);
    ...
    let concatenated = concatenate_expansion_output(shape, &data.tiles);
    let output = host_commit_u16(DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &concatenated);
    ...
    let transcript = host_commit_fields(DOMAIN_MLP_EXPANSION_GROUP_TRANSCRIPT, &transcript_fields);
...
fn load_join_data(arguments: &Arguments) -> JoinData {
    let group_manifest_path = arguments
        .tile_dir
        .join(format!("zkgpt_mlp_expansion_l{:02}.manifest.json", arguments.layer));
    ...
    let tile_outputs = split_expansion_output(arguments.shape, &combined);
    ...
    for (tile, output) in tile_outputs.into_iter().enumerate() {
        let manifest_path = arguments.tile_dir.join(format!(
            "zkgpt_mlp_expansion_l{:02}_t{tile:02}.commitments.txt",
            arguments.layer
        ));
        let manifest = parse_tile_manifest(&manifest_path);
        assert_eq!(manifest.layer, arguments.layer, "MLP expansion tile layer mismatch");
        assert_eq!(manifest.tile, tile, "MLP expansion tile index mismatch");
        ...
        let output_digest = host_commit_u16(DOMAIN_MLP_EXPANSION_TILE_OUTPUT, &output);
...
fn build_join(builder: &mut JoinBuilder, shape: Shape) {
    ...
    for tile in 0..shape.num_tiles {
        ...
        let output_digest =
            circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_TILE_OUTPUT, &output);
    ...
    let parameters =
        circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_GROUP_PARAMETERS, &parameter_fields);
    ...
    let output_digest = circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &output);
    ...
    let group_transcript =
        circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_GROUP_TRANSCRIPT, &group_fields);
...
fn verify_child_proofs(arguments: &Arguments, data: &JoinData) -> f64 {
    ...
    let vk_path = arguments
        .tile_dir
        .join(format!("zkgpt_mlp_expansion_l{:02}.shared.vk.bin", arguments.layer));
    ...
    for tile in &data.tiles {
        let proof_path = arguments.tile_dir.join(format!(
            "zkgpt_mlp_expansion_l{:02}_t{:02}.proof.bin",
        ...
        assert_eq!(proof.shard_proofs.len(), 1, "MLP expansion proof must contain one shard");
...
fn write_join_manifest(
    ...
    let manifest = format!(
        concat!(
            ...
            "  \"child_transcripts\": [{}],\n",
            "  \"private_output_file\": \"zkgpt_mlp_expansion_l{:02}.output.private.bf16.bin\",\n",
    ...
    println!("MLP expansion join manifest: {}", path.display());
}
...
async fn prove_join(
    ...
    for chip in machine.chips() {
        if chip.included(&record) {
            ...
            assert!(
                rows <= max_rows,
                "{} needs {rows} rows, exceeding MLP expansion join maximum {max_rows}",
                chip.name()
            );
        }
    }
    ...
    let prover = simple_prover(verifier);
    let proof_shape =
        prover.shape_from_record(&record).expect("MLP expansion join has no proof shape");
    ...
    println!("MLP expansion join setup: elapsed={setup_seconds:.3}s");
    ...
    println!("MLP expansion join proof generated: elapsed={prove_seconds:.3}s");

    let verify_started = Instant::now();
    prover.verify(&vk, &proof).expect("generated MLP expansion join proof must verify");
    let verify_seconds = verify_started.elapsed().as_secs_f64();
    println!("MLP expansion join proof verified: elapsed={verify_seconds:.3}s");

    fs::create_dir_all(&arguments.output_dir).unwrap_or_else(|error| {
        panic!("failed to create {}: {error}", arguments.output_dir.display())
    });
    let stem = format!("zkgpt_mlp_expansion_join_l{:02}", arguments.layer);
    ...
    println!(
        "MLP expansion join artifacts: proof={} ({} bytes) vk={} ({} bytes)",
        proof_path.display(),
...
async fn main() {
    ...
    println!(
        "mode={:?} layer={} seq_len={} hidden={} expansion={} tiles={} tile_dir={}",
        arguments.mode,
        arguments.layer,
        arguments.shape.sequence_length,
        arguments.shape.hidden_size,
    ...
    println!(
        "MLP expansion join witness: child_outputs={} BF16 values; target_rows=2^{JOIN_MAX_LOG_ROWS}",
        arguments.shape.sequence_length * arguments.shape.expansion_size
    );
    ...
    println!("built MLP expansion join: ir_ops={} elapsed={build_seconds:.3}s", block.ops.len());
    ...
    let commitments = validate_join_data(arguments.shape, arguments.layer, &data);
    println!(
        "loaded and checked MLP expansion join data: elapsed={:.3}s",
    ...
    println!("compiled MLP expansion join: elapsed={compile_seconds:.3}s");
    ...
    executor.run().expect("valid MLP expansion join witness must execute");
    let execute_seconds = execute_started.elapsed().as_secs_f64();
    println!("executed MLP expansion join: elapsed={execute_seconds:.3}s");
    assert_eq!(
        executor.record.public_values.digest, commitments.transcript,
        "MLP expansion join circuit transcript differs from host transcript"
    );
    println!("MLP expansion join public digest matches all child transcripts");
...
mod tests {
    ...
    fn synthetic_data(shape: Shape) -> JoinData {
        let upstream = host_commit_u16(0x4001, &[1, 2, 3, 4]);
        let input = host_commit_u16(0x4002, &[5, 6, 7, 8]);
        let tiles = (0..shape.num_tiles)
            .map(|tile| {
                let parameters = host_commit_u16(0x4003, &[tile as u16, 11]);
    ...
    fn split_and_concatenate_preserve_token_major_order() {
        ...
        let combined = (0..shape.sequence_length * shape.expansion_size)
            .map(|value| value as u16)
            .collect::<Vec<_>>();
        let tiles = split_expansion_output(shape, &combined)
            .into_iter()
            .enumerate()
            ...
            .collect::<Vec<_>>();
        assert_eq!(concatenate_expansion_output(shape, &tiles), combined);
    }
    ...
    fn join_circuit_rejects_tampered_child_data() {
        ...
        different_input.input = host_commit_u16(0x4002, &[9, 8, 7, 6]);
