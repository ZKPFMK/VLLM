...
const DOMAIN_MLP_PROJECTION_PARAMETERS: u32 = 0x1501;
...
impl Shape {
    ...
    fn validate(self) {
        assert!(self.sequence_length > 0, "MLP projection sequence length must be nonzero");
        assert!(self.hidden_size > 0, "MLP projection output size must be nonzero");
        assert!(self.expansion_size > 0, "MLP projection input size must be nonzero");
        assert!(self.num_tiles > 0, "MLP projection tile count must be nonzero");
        assert_eq!(
            self.expansion_size,
            3 * self.hidden_size,
...
impl EventCounts {
    fn for_shape(shape: Shape) -> Self {
        ...
        Self { mul: outputs * shape.expansion_size, add_sub: outputs * (shape.expansion_size - 1) }
    }

    fn total(self) -> usize {
        self.mul + self.add_sub
    }
}
...
fn parse_arguments() -> Arguments {
    ...
    let mut expansion_dir = std::env::var_os("SP1_ZKGPT_MLP_EXPANSION_OUTPUT_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("sp1-zkgpt-mlp-expansion-output"));
    let mut join_dir = std::env::var_os("SP1_ZKGPT_MLP_EXPANSION_JOIN_DIR").map(PathBuf::from);
    let mut data_dir = std::env::var_os("SP1_ZKGPT_LIKE_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(default_data_dir);
    let mut output_dir = std::env::var_os("SP1_ZKGPT_MLP_PROJECTION_OUTPUT_DIR").map(PathBuf::from);
    ...
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            ...
            Some("--expansion-dir") => {
                expansion_dir = PathBuf::from(
                    arguments.next().unwrap_or_else(|| panic!("--expansion-dir requires a value")),
                );
            }
            Some("--join-dir") => {
    ...
    if !synthetic {
        ...
        assert_eq!(
            ...
            "real MLP projection data uses the shape-aware full 30 x 2304 x 768 plan"
        );
    }
    let join_dir = join_dir.unwrap_or_else(|| expansion_dir.clone());
    let output_dir =
        output_dir.unwrap_or_else(|| std::env::temp_dir().join("sp1-zkgpt-mlp-projection-output"));
    Arguments {
        mode,
        shape,
        layer,
...
fn extract_weight_tile(full_weight: &[u16], shape: Shape, tile: usize) -> Vec<u16> {
    assert_eq!(full_weight.len(), shape.expansion_size * shape.hidden_size);
    assert!(tile < shape.num_tiles);
    let width = shape.tile_width();
    let start = tile * width;
    let mut weight = Vec::with_capacity(shape.expansion_size * width);
...
fn verify_upstream_join(arguments: &Arguments) -> (UpstreamCommitments, f64) {
    ...
    let manifest_path = arguments
        .join_dir
        .join(format!("zkgpt_mlp_expansion_join_l{:02}.manifest.json", arguments.layer));
    ...
    let vk_path =
        arguments.join_dir.join(format!("zkgpt_mlp_expansion_join_l{:02}.vk.bin", arguments.layer));
    let proof_path = arguments
        .join_dir
        .join(format!("zkgpt_mlp_expansion_join_l{:02}.proof.bin", arguments.layer));
    ...
    assert_eq!(proof.shard_proofs.len(), 1, "MLP expansion join must contain one shard");
    ...
    assert_eq!(
        ...
        "MLP expansion join public digest differs from its manifest"
    );
    let started = Instant::now();
    prover.verify(&vk, &proof).expect("upstream MLP expansion join proof must verify");
    let seconds = started.elapsed().as_secs_f64();
    println!("upstream MLP expansion join verified: elapsed={seconds:.3}s");
    (upstream, seconds)
}

fn load_source_data(arguments: &Arguments) -> (SourceData, f64) {
    if arguments.synthetic {
        let input = (0..arguments.shape.sequence_length * arguments.shape.expansion_size)
            .map(|index| [0x3f80, 0x4000, 0xbf80, 0x3f00][index % 4])
            .collect::<Vec<_>>();
        let upstream = UpstreamCommitments {
            output: host_commit_u16(DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &input),
            transcript: host_commit_u16(0x15ff, &[arguments.layer as u16]),
        };
        return (
            SourceData {
                input,
                full_weight: vec![
                    0x3f80;
                    arguments.shape.expansion_size * arguments.shape.hidden_size
                ],
                upstream,
            },
            0.0,
    ...
    let input_path = arguments
        .expansion_dir
        .join(format!("zkgpt_mlp_expansion_l{:02}.output.private.bf16.bin", arguments.layer));
    let input = read_bf16_binary(&input_path);
    assert_eq!(input.len(), arguments.shape.sequence_length * arguments.shape.expansion_size);
    let input_digest = host_commit_u16(DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &input);
    assert_eq!(input_digest, upstream.output, "private expansion output differs from join proof");

    let weight_path = arguments
        .data_dir
        .join(format!("layer-{:02}/mlp_projection_weight.bf16.bin", arguments.layer));
    let full_weight = read_bf16_binary(&weight_path);
    assert_eq!(full_weight.len(), arguments.shape.expansion_size * arguments.shape.hidden_size);
...
fn compute_tile_commitments(
    ...
    let input = host_commit_u16(DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &data.input);
    assert_eq!(input, data.upstream.output, "MLP projection input differs from expansion output");
    let parameters = host_commit_u16(DOMAIN_MLP_PROJECTION_PARAMETERS, &data.weight);
    let output = host_commit_u16(DOMAIN_MLP_PROJECTION_TILE_OUTPUT, output);
...
fn build_tile(builder: &mut TileBuilder, shape: Shape) {
    ...
    let expected_input: [Felt<SP1Field>; DIGEST_SIZE] =
        builder.hint_felts_v2(DIGEST_SIZE).try_into().unwrap();
    let input = builder.hint_felts_v2(shape.sequence_length * shape.expansion_size);
    let weight = builder.hint_felts_v2(shape.expansion_size * shape.tile_width());

    let input_digest = circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &input);
    for (&computed, &expected) in input_digest.iter().zip(&expected_input) {
        builder.assert_felt_eq(computed, expected);
    }
    let parameters = circuit_commit_fields(builder, DOMAIN_MLP_PROJECTION_PARAMETERS, &weight);
    let output = builder.bf16_linear_rows_no_bias(&input, shape.expansion_size, &weight);
    let output_digest = circuit_commit_fields(builder, DOMAIN_MLP_PROJECTION_TILE_OUTPUT, &output);
    ...
    let transcript =
        circuit_commit_fields(builder, DOMAIN_MLP_PROJECTION_TILE_TRANSCRIPT, &transcript_fields);
...
fn print_estimate(shape: Shape, events: EventCounts) {
    ...
    println!(
        "MLP projection tile: seq_len={} input={} tile_output={} full_output={} tiles={}",
        shape.sequence_length,
        shape.expansion_size,
        shape.tile_width(),
        shape.hidden_size,
    ...
    println!(
        "events per tile: mul={} add_sub={} total={}",
        events.mul,
        events.add_sub,
        events.total()
    );
...
fn write_tile_manifest(arguments: &Arguments, tile: usize, commitments: TileCommitments) {
    ...
    let path = arguments
        .output_dir
        .join(format!("zkgpt_mlp_projection_l{:02}_t{tile:02}.commitments.txt", arguments.layer));
...
fn execute_tile(
    ...
    let output =
        reference_linear_rows_no_bias(&data.input, arguments.shape.expansion_size, &data.weight);
    let commitments =
    ...
    executor.run().expect("valid MLP projection tile witness must execute");
    let execute_seconds = execute_started.elapsed().as_secs_f64();
    println!("executed MLP projection tile: tile={tile} elapsed={execute_seconds:.3}s");
    ...
    assert_eq!(
        ...
        "MLP projection circuit transcript differs from host transcript"
    );
...
async fn prove_tiles(
    ...
    println!("MLP projection shared setup: elapsed={setup_seconds:.3}s");
    ...
    let vk_stem = if arguments.all_tiles {
        format!("zkgpt_mlp_projection_l{:02}.shared", arguments.layer)
    } else {
        format!("zkgpt_mlp_projection_l{:02}_t{:02}", arguments.layer, tiles[0])
    };
    ...
    println!("shared MLP projection verifying key: {} ({vk_bytes} bytes)", vk_path.display());

    let batch_started = Instant::now();
    let mut completed = Vec::with_capacity(tiles.len());
    for (position, &tile) in tiles.iter().enumerate() {
        println!("MLP projection tile {}/{}: tile={tile}", position + 1, tiles.len());
        let execution = execute_tile(program.clone(), arguments, events, source, tile);
        ...
        let proof_shape = prover
            ...
            .expect("MLP projection tile machine has no proof shape");
        let trace_area = proof_shape.preprocessed_area + proof_shape.main_area;
        assert!(
            trace_area <= DEFAULT_MAX_TRACE_AREA,
            "MLP projection leaf trace area {trace_area} exceeds shape-aware limit {DEFAULT_MAX_TRACE_AREA}"
        );
        ...
        println!("MLP projection proof generated: tile={tile} elapsed={prove_seconds:.3}s");
        let verify_started = Instant::now();
        prover.verify(&vk, &proof).expect("generated MLP projection tile proof must verify");
        let verify_seconds = verify_started.elapsed().as_secs_f64();
        println!("MLP projection proof verified: tile={tile} elapsed={verify_seconds:.3}s");

        let proof_file =
            format!("zkgpt_mlp_projection_l{:02}_t{tile:02}.proof.bin", arguments.layer);
        ...
        println!("MLP projection proof artifact: {} ({proof_bytes} bytes)", proof_path.display());
        completed.push(CompletedTile {
            output,
            summary: TileSummary {
                tile,
...
fn compute_group(
    ...
    assert_eq!(completed.len(), shape.num_tiles, "MLP projection group requires every tile");
    ...
    for (expected_tile, tile) in completed.iter().enumerate() {
        assert_eq!(tile.summary.tile, expected_tile, "MLP projection tiles must be ordered");
    ...
    let output_digest = host_commit_u16(DOMAIN_MLP_PROJECTION_GROUP_OUTPUT, &output);
    let mut fields = [
        PROTOCOL_VERSION,
    ...
    let transcript = host_commit_fields(DOMAIN_MLP_PROJECTION_GROUP_TRANSCRIPT, &fields);
    (
        output,
...
fn write_group_artifacts(
    ...
    let stem = format!("zkgpt_mlp_projection_l{:02}", arguments.layer);
    ...
    writeln!(manifest, "  \"stage\": \"mlp_projection_group\",").unwrap();
    ...
    println!("MLP projection private output: {}", output_path.display());
    println!("MLP projection group manifest: {}", manifest_path.display());
    println!("MLP projection output commitment: {}", digest_hex(&commitments.output));
    println!("MLP projection group transcript: {}", digest_hex(&commitments.transcript));
}
...
async fn main() {
    ...
    println!("built MLP projection tile: ir_ops={} elapsed={build_seconds:.3}s", block.ops.len());
    ...
    println!(
        "loaded MLP projection source: input={} weight={} elapsed={:.3}s",
        source.input.len(),
    ...
    println!("compiled MLP projection tile: elapsed={compile_seconds:.3}s");
...
mod tests {
    ...
    fn weight_tile_preserves_row_major_output_columns() {
        ...
        let full = (0..shape.expansion_size * shape.hidden_size)
            .map(|value| value as u16)
            .collect::<Vec<_>>();
        let tile = extract_weight_tile(&full, shape, 1);
        let width = shape.tile_width();
        for input_feature in 0..shape.expansion_size {
            let expected_start = input_feature * shape.hidden_size + width;
            assert_eq!(
    ...
    #[test]
    fn projection_circuit_rejects_input_not_bound_to_expansion_join() {
        let shape = Shape::small();
        let input = (0..shape.sequence_length * shape.expansion_size)
            .map(|index| [0x3f80, 0x4000, 0xbf80, 0x3f00][index % 4])
            .collect::<Vec<_>>();
        let upstream = UpstreamCommitments {
            output: host_commit_u16(DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &input),
            transcript: host_commit_u16(0x15ff, &[0]),
        };
        let valid = TileData {
            input,
            weight: vec![0x3f80; shape.expansion_size * shape.tile_width()],
            upstream,
        };
        let output =
            reference_linear_rows_no_bias(&valid.input, shape.expansion_size, &valid.weight);
        ...
        modified.input[0] ^= 1;
        assert!(!execute(&modified).0, "input not committed by expansion join must fail");
    }
}
