...
use sp1_recursion_executor::{
    ...
    Bf16UnaryEvent, Bf16UnaryOpcode, Bf16UnaryWitness, ExecutionRecord, Executor, RecursionProgram,
...
const MLP_EXPANSION_TILE_STAGE: u32 = 6;
const MLP_EXPANSION_GROUP_STAGE: u32 = 7;
const DOMAIN_LN2_OUTPUT: u32 = 0x1302;
const DOMAIN_MLP_EXPANSION_PARAMETERS: u32 = 0x1401;
const DOMAIN_MLP_EXPANSION_TILE_OUTPUT: u32 = 0x1402;
const DOMAIN_MLP_EXPANSION_TILE_TRANSCRIPT: u32 = 0x1403;
const DOMAIN_MLP_EXPANSION_GROUP_PARAMETERS: u32 = 0x1411;
const DOMAIN_MLP_EXPANSION_GROUP_OUTPUT: u32 = 0x1412;
const DOMAIN_MLP_EXPANSION_GROUP_TRANSCRIPT: u32 = 0x1413;

const PROOF_LOG_BLOWUP: usize = 1;
const FULL_MAX_LOG_ROWS: usize = 19;
const SMALL_MAX_LOG_ROWS: usize = 16;
const UPSTREAM_LN2_MAX_LOG_ROWS: usize = 16;
...
impl Shape {
    ...
    fn small() -> Self {
        Self { sequence_length: 2, hidden_size: 8, expansion_size: 24, num_tiles: 3 }
    }

    fn validate(self) {
        assert!(self.sequence_length > 0, "MLP expansion sequence length must be nonzero");
        assert!(self.hidden_size > 0, "MLP expansion input size must be nonzero");
        assert!(self.expansion_size > 0, "MLP expansion output size must be nonzero");
        assert!(self.num_tiles > 0, "MLP expansion tile count must be nonzero");
        assert_eq!(
            self.expansion_size,
            3 * self.hidden_size,
            "zkGPT MLP expansion width must be three times hidden size"
        );
        assert_eq!(
            self.expansion_size % self.num_tiles,
            0,
            "MLP expansion width must be divisible by tile count"
        );
    }
...
impl EventCounts {
    fn for_shape(shape: Shape) -> Self {
        ...
        Self {
            mul: outputs * shape.hidden_size,
            add_sub: outputs * (shape.hidden_size - 1),
            unary: outputs,
        }
    }

    fn total(self) -> usize {
        self.mul + self.add_sub + self.unary
    }
}
...
fn parse_arguments() -> Arguments {
    ...
    let mut ln2_dir = std::env::var_os("SP1_ZKGPT_LN2_OUTPUT_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("sp1-zkgpt-ln2-output"));
    let mut data_dir = std::env::var_os("SP1_ZKGPT_LIKE_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(default_data_dir);
    let mut output_dir = std::env::var_os("SP1_ZKGPT_MLP_EXPANSION_OUTPUT_DIR").map(PathBuf::from);
    ...
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            ...
            Some("--ln2-dir") => {
                ln2_dir = PathBuf::from(
                    arguments.next().unwrap_or_else(|| panic!("--ln2-dir requires a value")),
                );
            }
            Some("--data-dir") => {
    ...
    if !synthetic {
        let full = Shape::full();
        assert_eq!(
            (shape.sequence_length, shape.hidden_size, shape.expansion_size, shape.num_tiles,),
            (DEFAULT_SEQUENCE_LENGTH, DEFAULT_HIDDEN_SIZE, DEFAULT_EXPANSION_SIZE, full.num_tiles,),
            "real MLP expansion data uses the shape-aware full 30 x 768 x 2304 plan"
        );
    }
    let output_dir =
        output_dir.unwrap_or_else(|| std::env::temp_dir().join("sp1-zkgpt-mlp-expansion-output"));
    Arguments { mode, shape, layer, tile, all_tiles, synthetic, ln2_dir, data_dir, output_dir }
}
...
fn extract_weight_tile(full_weight: &[u16], shape: Shape, tile: usize) -> Vec<u16> {
    assert_eq!(full_weight.len(), shape.hidden_size * shape.expansion_size);
    ...
    for row in full_weight.chunks_exact(shape.expansion_size) {
...
fn reference_expansion_gelu(input: &[u16], input_features: usize, weight: &[u16]) -> Vec<u16> {
    reference_linear_rows_no_bias(input, input_features, weight)
        .into_iter()
        .map(|value| Bf16UnaryWitness::new(Bf16UnaryOpcode::GeluNew, value).output)
        .collect()
}

fn verify_upstream_ln2(arguments: &Arguments) -> (UpstreamCommitments, f64) {
    type A = RecursionAir<SP1Field, 3, 2>;

    let manifest_path =
        arguments.ln2_dir.join(format!("zkgpt_ln2_l{:02}.manifest.json", arguments.layer));
    ...
    let verifier = ShardVerifier::from_basefold_parameters(
        ...
        UPSTREAM_LN2_MAX_LOG_ROWS as u32,
        UPSTREAM_LN2_MAX_LOG_ROWS,
        A::verillm_machine(),
    );
    let prover = simple_prover(verifier);
    let vk_path = arguments.ln2_dir.join(format!("zkgpt_ln2_l{:02}.vk.bin", arguments.layer));
    let proof_path = arguments.ln2_dir.join(format!("zkgpt_ln2_l{:02}.proof.bin", arguments.layer));
    ...
    assert_eq!(proof.shard_proofs.len(), 1, "LN2 proof must contain one shard");
    ...
    assert_eq!(
        ...
        "LN2 proof public digest differs from its manifest"
    );
    let started = Instant::now();
    prover.verify(&vk, &proof).expect("upstream LN2 proof must verify");
    let seconds = started.elapsed().as_secs_f64();
    println!("upstream LN2 proof verified: elapsed={seconds:.3}s");
    (upstream, seconds)
}

fn load_source_data(arguments: &Arguments) -> (SourceData, f64) {
    if arguments.synthetic {
        let input = (0..arguments.shape.sequence_length * arguments.shape.hidden_size)
            .map(|index| [0x3f80, 0x4000, 0xbf80, 0x3f00][index % 4])
            .collect::<Vec<_>>();
        let upstream = UpstreamCommitments {
            output: host_commit_u16(DOMAIN_LN2_OUTPUT, &input),
            transcript: host_commit_u16(0x14ff, &[arguments.layer as u16]),
        };
        return (
            SourceData {
                input,
                full_weight: vec![
                    0x3f80;
                    arguments.shape.hidden_size * arguments.shape.expansion_size
                ],
                upstream,
            },
            0.0,
    ...
    let (upstream, upstream_verify_seconds) = verify_upstream_ln2(arguments);
    let input_path = arguments
        .ln2_dir
        .join(format!("zkgpt_ln2_l{:02}.output.private.bf16.bin", arguments.layer));
    let input = read_bf16_binary(&input_path);
    assert_eq!(input.len(), arguments.shape.sequence_length * arguments.shape.hidden_size);
    let input_digest = host_commit_u16(DOMAIN_LN2_OUTPUT, &input);
    assert_eq!(input_digest, upstream.output, "private LN2 output differs from its proof");

    let weight_path = arguments
        .data_dir
        .join(format!("layer-{:02}/mlp_expansion_weight.bf16.bin", arguments.layer));
    let full_weight = read_bf16_binary(&weight_path);
    assert_eq!(full_weight.len(), arguments.shape.hidden_size * arguments.shape.expansion_size);
    (SourceData { input, full_weight, upstream }, upstream_verify_seconds)
}
...
fn compute_tile_commitments(
    ...
    let input = host_commit_u16(DOMAIN_LN2_OUTPUT, &data.input);
    assert_eq!(input, data.upstream.output, "MLP expansion input differs from LN2 output");
    let parameters = host_commit_u16(DOMAIN_MLP_EXPANSION_PARAMETERS, &data.weight);
    let output = host_commit_u16(DOMAIN_MLP_EXPANSION_TILE_OUTPUT, output);
    let transcript = host_commit_fields(
        DOMAIN_MLP_EXPANSION_TILE_TRANSCRIPT,
...
fn build_tile(builder: &mut TileBuilder, shape: Shape) {
    ...
    let input_digest = circuit_commit_fields(builder, DOMAIN_LN2_OUTPUT, &input);
    for (&computed, &expected) in input_digest.iter().zip(&expected_input) {
        builder.assert_felt_eq(computed, expected);
    }
    let parameters = circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_PARAMETERS, &weight);
    let expanded = builder.bf16_linear_rows_no_bias(&input, shape.hidden_size, &weight);
    let output = builder.bf16_gelu_new_rows(&expanded, shape.tile_width());
    let output_digest = circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_TILE_OUTPUT, &output);
    ...
    let transcript =
        circuit_commit_fields(builder, DOMAIN_MLP_EXPANSION_TILE_TRANSCRIPT, &transcript_fields);
...
fn print_estimate(shape: Shape, events: EventCounts) {
    ...
    println!(
        "MLP expansion tile: seq_len={} input={} tile_output={} full_output={} tiles={}",
        shape.sequence_length,
        shape.hidden_size,
        shape.tile_width(),
        shape.expansion_size,
    ...
    println!(
        "events per tile: mul={} add_sub={} unary={} total={}",
        events.mul,
        events.add_sub,
        events.unary,
        events.total()
...
fn write_tile_manifest(arguments: &Arguments, tile: usize, commitments: TileCommitments) {
    ...
    let path = arguments
        .output_dir
        .join(format!("zkgpt_mlp_expansion_l{:02}_t{tile:02}.commitments.txt", arguments.layer));
...
fn execute_tile(
    ...
    let output = reference_expansion_gelu(&data.input, arguments.shape.hidden_size, &data.weight);
    let commitments =
    ...
    println!(
        "host BF16 reference: tile={tile} output_values={} elapsed={reference_seconds:.3}s",
        output.len()
    );
    ...
    executor.run().expect("valid MLP expansion tile witness must execute");
    let execute_seconds = execute_started.elapsed().as_secs_f64();
    println!("executed MLP expansion tile: tile={tile} elapsed={execute_seconds:.3}s");
    ...
    assert_eq!(
        ...
        "MLP expansion circuit transcript differs from host transcript"
    );
...
async fn prove_tiles(
    ...
    println!("MLP expansion shared setup: elapsed={setup_seconds:.3}s");
    ...
    let vk_stem = if arguments.all_tiles {
        format!("zkgpt_mlp_expansion_l{:02}.shared", arguments.layer)
    } else {
        format!("zkgpt_mlp_expansion_l{:02}_t{:02}", arguments.layer, tiles[0])
    };
    ...
    println!("shared MLP expansion verifying key: {} ({vk_bytes} bytes)", vk_path.display());

    let batch_started = Instant::now();
    let mut completed = Vec::with_capacity(tiles.len());
    for (position, &tile) in tiles.iter().enumerate() {
        println!("MLP expansion tile {}/{}: tile={tile}", position + 1, tiles.len());
        let execution = execute_tile(program.clone(), arguments, events, source, tile);
        ...
        let proof_shape = prover
            ...
            .expect("MLP expansion tile machine has no proof shape");
        let trace_area = proof_shape.preprocessed_area + proof_shape.main_area;
        assert!(
            trace_area <= DEFAULT_MAX_TRACE_AREA,
            "MLP expansion leaf trace area {trace_area} exceeds shape-aware limit {DEFAULT_MAX_TRACE_AREA}"
        );
        ...
        println!("MLP expansion proof generated: tile={tile} elapsed={prove_seconds:.3}s");
        let verify_started = Instant::now();
        prover.verify(&vk, &proof).expect("generated MLP expansion tile proof must verify");
        let verify_seconds = verify_started.elapsed().as_secs_f64();
        println!("MLP expansion proof verified: tile={tile} elapsed={verify_seconds:.3}s");

        let proof_file =
            format!("zkgpt_mlp_expansion_l{:02}_t{tile:02}.proof.bin", arguments.layer);
        ...
        println!("MLP expansion proof artifact: {} ({proof_bytes} bytes)", proof_path.display());
        completed.push(CompletedTile {
            output,
            summary: TileSummary {
                tile,
...
fn compute_group(
    ...
    assert_eq!(completed.len(), shape.num_tiles, "MLP expansion group requires every tile");
    ...
    for (expected_tile, tile) in completed.iter().enumerate() {
        assert_eq!(tile.summary.tile, expected_tile, "MLP expansion tiles must be ordered");
        assert_eq!(tile.summary.commitments.input, upstream.output, "tile inputs differ");
        assert_eq!(
            tile.summary.commitments.upstream, upstream.transcript,
            "tile upstream transcripts differ"
        );
    ...
    let parameters = host_commit_fields(DOMAIN_MLP_EXPANSION_GROUP_PARAMETERS, &parameter_fields);
    let tile_width = shape.tile_width();
    let mut output = Vec::with_capacity(shape.sequence_length * shape.expansion_size);
    ...
    let output_digest = host_commit_u16(DOMAIN_MLP_EXPANSION_GROUP_OUTPUT, &output);
    let mut fields = [
        PROTOCOL_VERSION,
        MLP_EXPANSION_GROUP_STAGE,
        layer as u32,
        shape.sequence_length as u32,
        shape.hidden_size as u32,
        shape.expansion_size as u32,
    ...
    let transcript = host_commit_fields(DOMAIN_MLP_EXPANSION_GROUP_TRANSCRIPT, &fields);
    (
        output,
...
fn write_group_artifacts(
    ...
    let stem = format!("zkgpt_mlp_expansion_l{:02}", arguments.layer);
    ...
    writeln!(manifest, "  \"stage\": \"mlp_expansion_gelu_group\",").unwrap();
    ...
    writeln!(manifest, "  \"expansion_size\": {},", arguments.shape.expansion_size).unwrap();
    ...
    writeln!(manifest, "  \"upstream_verify_seconds\": {:.6},", metrics.upstream_verify_seconds)
        .unwrap();
    ...
    println!("MLP expansion private output: {}", output_path.display());
    println!("MLP expansion group manifest: {}", manifest_path.display());
    println!("MLP expansion output commitment: {}", digest_hex(&commitments.output));
    println!("MLP expansion group transcript: {}", digest_hex(&commitments.transcript));
}
...
async fn main() {
    ...
    println!(
        "mode={:?} layer={} tiles={tiles:?} reuse_setup={} synthetic={}",
        arguments.mode, arguments.layer, arguments.all_tiles, arguments.synthetic
    );
    ...
    println!("built MLP expansion tile: ir_ops={} elapsed={build_seconds:.3}s", block.ops.len());
    ...
    let (source, upstream_verify_seconds) = load_source_data(&arguments);
    println!(
        "loaded MLP expansion source: input={} weight={} elapsed={:.3}s",
        source.input.len(),
    ...
    println!("compiled MLP expansion tile: elapsed={compile_seconds:.3}s");

    let (completed, mut metrics) = if arguments.mode == Mode::Prove {
        prove_tiles(program, &arguments, events, &source, &tiles).await
    } else {
    ...
    metrics.upstream_verify_seconds = upstream_verify_seconds;
...
mod tests {
    ...
    #[test]
    fn weight_tile_preserves_row_major_output_columns() {
        let shape = Shape::small();
        let full = (0..shape.hidden_size * shape.expansion_size)
            .map(|value| value as u16)
            .collect::<Vec<_>>();
        let tile = extract_weight_tile(&full, shape, 1);
        let width = shape.tile_width();
        for input_feature in 0..shape.hidden_size {
            let expected_start = input_feature * shape.expansion_size + width;
            assert_eq!(
                &tile[input_feature * width..(input_feature + 1) * width],
                &full[expected_start..expected_start + width]
            );
        }
    }

    #[test]
    fn expansion_circuit_rejects_private_input_not_bound_to_ln2() {
        let shape = Shape::small();
        ...
        let upstream = UpstreamCommitments {
            output: host_commit_u16(DOMAIN_LN2_OUTPUT, &input),
            transcript: host_commit_u16(0x14ff, &[0]),
        };
        let valid = TileData {
            input,
        ...
        let output = reference_expansion_gelu(&valid.input, shape.hidden_size, &valid.weight);
        let expected = compute_tile_commitments(shape, DEFAULT_LAYER, 0, &valid, &output);
        ...
        let execute = |data: &TileData| {
            ...
            executor.witness_stream = witness_stream(&arguments, 0, data).into();
        ...
        modified.input[0] ^= 1;
        assert!(!execute(&modified).0, "input not committed by LN2 proof must fail");
    }
}
