...
impl Shape {
    ...
    fn validate(self) {
        assert!(self.sequence_length > 0, "c_proj join sequence length must be nonzero");
        assert!(self.hidden_size > 0, "c_proj join hidden size must be nonzero");
        assert!(self.num_tiles > 0, "c_proj join tile count must be nonzero");
        assert_eq!(
            self.hidden_size % self.num_tiles,
            0,
            "c_proj join hidden size must be divisible by tile count"
        );
    }
...
fn parse_arguments() -> Arguments {
    ...
    let mut tile_dir = std::env::var_os("SP1_ZKGPT_C_PROJ_OUTPUT_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("sp1-zkgpt-c-proj-output"));
    let mut output_dir = std::env::var_os("SP1_ZKGPT_C_PROJ_JOIN_DIR").map(PathBuf::from);
...
fn parse_tile_manifest(path: &Path) -> TileManifest {
    ...
    assert_eq!(required_field(&fields, "stage", path), "attention_c_proj_tile");
...
fn split_c_proj_output(shape: Shape, combined: &[u16]) -> Vec<Vec<u16>> {
...
fn concatenate_c_proj_output(shape: Shape, tiles: &[JoinTileData]) -> Vec<u16> {
...
fn validate_join_data(shape: Shape, layer: usize, data: &JoinData) -> GroupCommitments {
    assert_eq!(data.tiles.len(), shape.num_tiles, "c_proj join requires every tile");
    ...
    for (expected_tile, tile) in data.tiles.iter().enumerate() {
        assert_eq!(tile.tile, expected_tile, "c_proj join tiles must be ordered from zero");
        let output = host_commit_u16(DOMAIN_C_PROJ_TILE_OUTPUT, &tile.output);
    ...
    let concatenated = concatenate_c_proj_output(shape, &data.tiles);
    let output = host_commit_u16(DOMAIN_C_PROJ_GROUP_OUTPUT, &concatenated);
    ...
    let transcript = host_commit_fields(DOMAIN_C_PROJ_GROUP_TRANSCRIPT, &transcript_fields);
...
fn load_join_data(arguments: &Arguments) -> JoinData {
    let group_manifest_path =
        arguments.tile_dir.join(format!("zkgpt_c_proj_l{:02}.manifest.json", arguments.layer));
    ...
    let tile_outputs = split_c_proj_output(arguments.shape, &combined);
    ...
    for (tile, output) in tile_outputs.into_iter().enumerate() {
        ...
        assert_eq!(manifest.layer, arguments.layer, "c_proj tile layer mismatch");
        assert_eq!(manifest.tile, tile, "c_proj tile index mismatch");
        assert_eq!(manifest.upstream, expected_upstream, "c_proj upstream transcripts differ");
        assert_eq!(manifest.input, expected_input, "c_proj tile input commitments differ");
        let output_digest = host_commit_u16(DOMAIN_C_PROJ_TILE_OUTPUT, &output);
...
fn build_join(builder: &mut JoinBuilder, shape: Shape) {
    ...
    let parameters =
        circuit_commit_fields(builder, DOMAIN_C_PROJ_GROUP_PARAMETERS, &parameter_fields);
    ...
    let output_digest = circuit_commit_fields(builder, DOMAIN_C_PROJ_GROUP_OUTPUT, &output);
    ...
    let group_transcript =
        circuit_commit_fields(builder, DOMAIN_C_PROJ_GROUP_TRANSCRIPT, &group_fields);
...
fn verify_child_proofs(arguments: &Arguments, data: &JoinData) -> f64 {
    ...
    let vk_path =
        arguments.tile_dir.join(format!("zkgpt_c_proj_l{:02}.shared.vk.bin", arguments.layer));
    ...
    for tile in &data.tiles {
        let proof_path = arguments
            .tile_dir
            .join(format!("zkgpt_c_proj_l{:02}_t{:02}.proof.bin", arguments.layer, tile.tile));
        ...
        assert_eq!(proof.shard_proofs.len(), 1, "c_proj tile proof must contain one shard");
...
fn write_join_manifest(
    ...
    let manifest = format!(
        concat!(
            ...
            "  \"child_transcripts\": [{}],\n",
            "  \"private_output_file\": \"zkgpt_c_proj_l{:02}.output.private.bf16.bin\",\n",
    ...
    let path = arguments
        .output_dir
        .join(format!("zkgpt_c_proj_join_l{:02}.manifest.json", arguments.layer));
    fs::write(&path, manifest)
        .unwrap_or_else(|error| panic!("failed to write {}: {error}", path.display()));
    println!("c_proj join manifest: {}", path.display());
}
...
async fn prove_join(
    ...
    for chip in machine.chips() {
        if chip.included(&record) {
            ...
            assert!(
                rows <= max_rows,
                "{} needs {rows} rows, exceeding c_proj join maximum {max_rows}",
                chip.name()
            );
        }
    }
    ...
    let prover = simple_prover(verifier);
    let proof_shape = prover.shape_from_record(&record).expect("c_proj join has no proof shape");
    ...
    println!("c_proj join setup: elapsed={setup_seconds:.3}s");
    ...
    println!("c_proj join proof generated: elapsed={prove_seconds:.3}s");

    let verify_started = Instant::now();
    prover.verify(&vk, &proof).expect("generated c_proj join proof must verify");
    let verify_seconds = verify_started.elapsed().as_secs_f64();
    println!("c_proj join proof verified: elapsed={verify_seconds:.3}s");

    fs::create_dir_all(&arguments.output_dir).unwrap_or_else(|error| {
        panic!("failed to create {}: {error}", arguments.output_dir.display())
    });
    let stem = format!("zkgpt_c_proj_join_l{:02}", arguments.layer);
    ...
    println!(
        "c_proj join proof artifacts: proof={} ({} bytes) vk={} ({} bytes)",
        proof_path.display(),
...
async fn main() {
    ...
    println!(
        "mode={:?} layer={} seq_len={} hidden={} tiles={} tile_dir={}",
        arguments.mode,
        arguments.layer,
        arguments.shape.sequence_length,
        arguments.shape.hidden_size,
    ...
    println!(
        "c_proj join witness: child_outputs={} BF16 values; target_rows=2^{JOIN_MAX_LOG_ROWS}",
    ...
    println!("built c_proj join: ir_ops={} elapsed={build_seconds:.3}s", block.ops.len());
    ...
    let commitments = validate_join_data(arguments.shape, arguments.layer, &data);
    println!(
        "loaded and checked c_proj join data: elapsed={:.3}s",
    ...
    println!("compiled c_proj join: elapsed={compile_seconds:.3}s");
    ...
    executor.run().expect("valid c_proj join witness must execute");
    let execute_seconds = execute_started.elapsed().as_secs_f64();
    println!("executed c_proj join: elapsed={execute_seconds:.3}s");
    assert_eq!(
        executor.record.public_values.digest, commitments.transcript,
        "c_proj join circuit transcript differs from host transcript"
    );
    println!("c_proj join public digest matches all child transcripts");
...
mod tests {
    ...
    fn synthetic_data(shape: Shape) -> JoinData {
        let upstream = host_commit_u16(0x3001, &[1, 2, 3, 4]);
        let input = host_commit_u16(0x3002, &[5, 6, 7, 8]);
        let tiles = (0..shape.num_tiles)
            .map(|tile| {
                let parameters = host_commit_u16(0x3003, &[tile as u16, 11]);
    ...
    fn split_and_concatenate_preserve_token_major_order() {
        ...
        let combined = (0..shape.sequence_length * shape.hidden_size)
            ...
            .collect::<Vec<_>>();
        let tiles = split_c_proj_output(shape, &combined)
            .into_iter()
            .enumerate()
            ...
            .collect::<Vec<_>>();
        assert_eq!(concatenate_c_proj_output(shape, &tiles), combined);
    }
    ...
    fn join_circuit_rejects_tampered_child_data() {
        ...
        different_input.input = host_commit_u16(0x3002, &[9, 8, 7, 6]);
